...
<head>
    ...
    <meta charset='UTF-8' />
    <title>Fee Branch Cmty Report</title>
</head>
...
<body>
        ...
        <ul class="mainnav">
            ...
            <li class="hassubs active"><a href="javascript:void(0)">Reports</a>
                <ul class="dropdown">
                    ...
                    <li class="subs hassubs active"><a href="javascript:void(0)">Fee&ensp;Paid</a>
                        <ul class="dropdown">
                            <li class="subs sub2"><a href="freport1.php">Branch</a></li>
                            <li class="subs sub2"><a href="freport2.php">Branch&ensp;Mark</a></li>
                            <li class="subs active"><a href="freport3.php">Branch_Community</a></li>
    ...
    <div class='container'>
        <div class="content">
            ...
            <?php
    $startdate=$enddate=$course=$community="";
...
?>
                    ...
                    <table class="table1">
                        <tr>
                            <th>
                                ...
                                Community:<select name="community" required>
                                    <option value="OC" <?php if($community == "OC")echo "selected";?>>OC</option>
                                    <option value="BC(M)" <?php if($community == "BC(M)")echo "selected";?>>BC(M)</option>
                                    <option value="BC" <?php if($community == "BC")echo "selected";?>>BC</option>
                                    <option value="MBC" <?php if($community == "MBC")echo "selected";?>>MBC</option>
                                    <option value="DNC" <?php if($community == "DNC")echo "selected";?>>DNC</option>
                                    <option value="SC(A)" <?php if($community == "SC(A)")echo "selected";?>>SC(A)</option>
                                    <option value="SC" <?php if($community == "SC")echo "selected";?>>SC</option>
                                    <option value="ST" <?php if($community == "ST")echo "selected";?>>ST</option>
                                </select>
                            </th>
                            <td>
    ...
    if(isset($_POST["report"])){
        if(isset($_POST["report"])){
            ...
            $course=$_POST["course"];
            $community=$_POST["community"];
            $sql="SELECT * FROM feetable where  (feedate BETWEEN '$startdate' AND '$enddate') AND course = '$course' AND community = '$community' AND feedate <> '' ORDER BY cutoff DESC";
        }
        if($_POST['startdate'] == ""){
            $sql="SELECT * FROM feetable where course = '$course' AND community = '$community' AND feedate <> '' ORDER BY cutoff DESC";
        }
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        while($row=$result->fetch_assoc()){
         ...
         $cvdate1 = date("d-m-Y",strtotime($row["cvdate"]));
        echo "<tr>
             <td>
                 ".$sno++."
             </td>
